  ...
  try{
    properties([
      ...
      parameters([
        ...
        [$class: 'ListSubversionTagsParameterDefinition', credentialsId: 'munjal-gc-un-pw', defaultValue: '', maxTags: '', name: 'TagName', reverseByDate: true, reverseByName: false, tagsDir: 'https://github.com/BidClips/BidClips-TaxJar-API.git', tagsFilter: '']
      ])
    ])
    def PUBLISHTAG = ""
    def repoRegion = ""
...
docker images | grep bidclipstaxjarapi
            """
      }
    }
...
docker tag bidclipstaxjarapi:latest 566570633830.dkr.ecr.${repoRegion}.amazonaws.com/bidclips-taxjar-api:${PUBLISHTAG}
docker push 566570633830.dkr.ecr.${repoRegion}.amazonaws.com/bidclips-taxjar-api:${PUBLISHTAG}
        """
...
// docker tag bidclipstaxjarapi:latest 875588116685.dkr.ecr.us-east-1.amazonaws.com/bidclips-taxjar-api:${PUBLISHTAG}
// docker push 875588116685.dkr.ecr.us-east-1.amazonaws.com/bidclips-taxjar-api:${PUBLISHTAG}
// docker image rmi -f 875588116685.dkr.ecr.us-east-1.amazonaws.com/bidclips-taxjar-api:${PUBLISHTAG}
//         """
//       }
    }
    stage('Cleanup'){
      sh """
      docker image rmi -f bidclipstaxjarapi:latest
      docker image rmi -f 566570633830.dkr.ecr.${repoRegion}.amazonaws.com/bidclips-taxjar-api:${PUBLISHTAG}
      """
      dir('BidClips-TaxJar-API') {
        deleteDir()
      }
